...
ab._MESSAGE_TYPEID_DEL_ROOM       = 6;
ab._MESSAGE_TYPEID_PUBLISH        = 7;
ab._MESSAGE_TYPEID_EVENT          = 8;
ab._MESSAGE_TYPEID_STATUS         = 9;
ab._MESSAGE_TYPEID_UNSUB_STATUS   = 10;
ab._MESSAGE_TYPEID_INITIALIZE     = 11;
ab._MESSAGE_TYPEID_DONE           = 12;
...
ab.Session = function (
   ...
   self._websocket_onmessage = onmessage;
   self._websocket_onstatus = onstatus;
   self._websocket_initialize = initialize;
   self._websocket_onnewroom = onnewroom;
   self._websocket_ondelroom = ondelroom;
   self._websocket_onoffer = onoffer;
   self._websocket_onanswer = onanswer;
   self._websocket_onicecandidate = onicecandidate;
   self._websocket_onready = onready;
   self._websocket_hangup = hangUp;
   ...
   self._websocket.onmessage = function (e)
   {
      ...
      else if (o[0] === ab._MESSAGE_TYPEID_EVENT) //request data = [MSG_CODE, room_id, data_object]
      {
         switch (o[2].type) {
            case 0:
               self._websocket_onmessage(o[1], o[2]);
            break;

            case 1:
               self._websocket_onoffer(o[2].offer);
            break;

            case 2:
               self._websocket_onanswer(o[2].answer);
            break;

            case 3:
               self._websocket_onicecandidate(o[2].iceCandidate);
            break;

            case 4:
               self._websocket_hangup();
            break;

            case 7:
               self._websocket_onready(o[2].video, o[1]);
            break;
         }
      }
      ...
      } else if (o[0] === ab._MESSAGE_TYPEID_STATUS) {
         self._websocket_onstatus(o[1], o[2]);
      } else if (o[0] === ab._MESSAGE_TYPEID_DONE) {
         self._websocket_initialize(o[1]);
      } else if (o[0] === ab._MESSAGE_TYPEID_SUBSCRIBE) {
         self._websocket_onnewroom(o[1], o[2]);
      } else if (o[0] === ab._MESSAGE_TYPEID_DEL_ROOM) {
         self._websocket_ondelroom(o[1]);
      }
   };
...
ab.Session.prototype.new_room = function (room_id, sender, recipient) {
   var self = this;
   let msg = [ab._MESSAGE_TYPEID_SUBSCRIBE, String(room_id), sender, recipient];
   self._send(msg);
};


ab.Session.prototype.del_room = function (room, sender_id, recipient_id) {
   var self = this;
   let msg = [ab._MESSAGE_TYPEID_DEL_ROOM, room, sender_id, recipient_id]
   self._send(msg);
};


ab.Session.prototype.publish = function (room_id, data) {
   var self = this;
   let msg = [ab._MESSAGE_TYPEID_PUBLISH, room_id, data];
   self._send(msg);
};

ab.Session.prototype.initialize = function(id, id_list, rooms_list) {
   var self = this;
   let msg = [ab._MESSAGE_TYPEID_INITIALIZE, id, id_list, rooms_list];
   self._send(msg);
}

export default ab.Session;
//exports.Session = ab.Session;